...
function Task(props) {
  ...
  return (
    <div id='task' className={state.isEditable ? 'editable' : ''}>
      ...
      <MuiIcons.Add className='action-btn' onClick={(e) => handleCount(e, 'add')}/>
      <MuiIcons.Remove className='action-btn' style={{color: 'darkred'}} onClick={(e) => {if(state.count) handleCount(e, 'remove')}}/>
      <MuiIcons.Edit className='action-btn' style={state.isEditable ? {color: 'white'}:{color: 'black'}} onClick={toggleEditable} />
    </div>
  );
}
